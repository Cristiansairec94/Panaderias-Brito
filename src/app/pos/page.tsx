    ...
    <div className="flex h-full w-full overflow-hidden bg-stone-100/70 relative">
      ...
      <div className={`fixed lg:static inset-y-0 right-0 z-40 w-full sm:w-[420px] lg:w-[460px] xl:w-[500px] 2xl:w-[540px] shrink-0 bg-white border-l-2 border-stone-200 flex flex-col h-full shadow-2xl transition-transform duration-300 ease-in-out relative overflow-hidden ${
        ...
        <div ref={customerPickerRef} className="p-2 sm:px-3 bg-gradient-to-r from-amber-50/90 via-stone-50 to-orange-50/70 border-b border-amber-200/80 shrink-0 relative">
          <div className="flex items-center justify-between gap-2">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10px] text-stone-500 font-medium truncate">
                  {selectedCustomer.type === "general" 
                    ...
                    : `${selectedCustomer.phone && selectedCustomer.phone !== "N/A" && selectedCustomer.phone !== "Sin teléfono" ? `📞 ${selectedCustomer.phone}` : "Cliente frecuente"}${selectedCustomer.notes ? ` • 📝 ${selectedCustomer.notes}` : ""}`}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1 shrink-0">
              ...
              <button
                ...
                title="Buscar o seleccionar cliente"
              >
          ...
          {/* Customer Dropdown Popover con Buscador Inteligente */}
          {isCustomerPickerOpen && (
            <div className="absolute left-2 right-2 top-full mt-1.5 z-50 bg-white rounded-2xl shadow-2xl border-2 border-amber-400/90 p-3 space-y-2.5 animate-in fade-in zoom-in-95 duration-150 max-h-[400px] flex flex-col">
              ...
              <div className="flex items-center justify-between pb-1 border-b border-stone-100">
                <span className="text-xs font-black text-stone-900 flex items-center gap-1.5">
                  <Users className="w-3.5 h-3.5 text-amber-600" /> Buscador de Clientes
                </span>
              ...
              <div className="relative">
                ...
                <input
                  type="text"
                  autoFocus
                  placeholder="Buscar por teléfono, nombre o característica..."
                  value={customerSearchQuery}
                  onChange={(e) => setCustomerSearchQuery(e.target.value)}
                  className="w-full pl-8 pr-7 py-2 bg-stone-100 rounded-xl text-xs font-bold text-stone-900 border border-stone-200 focus:outline-none focus:border-amber-500 focus:bg-white transition-all placeholder:text-stone-400"
                />
                {customerSearchQuery && (
                  <button
                    type="button"
                ...
                {filteredCustomers.filter((c) => c.id !== "cli-0").length === 0 && (
                  <div className="p-4 text-center space-y-1.5 bg-stone-50 rounded-xl border border-dashed border-stone-200">
                    ...
                    <button
                      ...
                      className="mt-1 px-3 py-1.5 bg-amber-500 hover:bg-amber-600 text-stone-950 rounded-xl text-xs font-black transition-all active:scale-95 shadow-xs"
                    >
              ...
              <div className="pt-2 border-t border-stone-100">
                <button
                  ...
                  className="w-full py-2.5 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-600 hover:to-orange-600 text-white font-black text-xs flex items-center justify-center gap-1.5 shadow-sm transition-all active:scale-95"
                >
                  <Plus className="w-3.5 h-3.5" />
