...
export default function ProductosPage() {
  ...
  const handleSubmitForm = (e: React.FormEvent) => {
    ...
    if (modalMode === "create") {
      ...
      showToast(`¡Producto "${created.name}" [${created.barcode || created.code}] creado con éxito!`);
                  ...
                  <div className="absolute top-3 left-3 flex flex-col gap-1 z-10">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      {(product.barcode || product.code) && (
                        <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] font-black font-mono tracking-wider bg-stone-950/90 text-amber-300 border border-amber-500/40 backdrop-blur-md shadow-md">
        ...
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-3 sm:p-6 overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-white rounded-3xl sm:rounded-[32px] max-w-lg w-full p-5 sm:p-8 shadow-2xl border border-stone-200 relative my-auto max-h-[92vh] overflow-y-auto animate-in zoom-in-95 duration-200">
            ...
            <form onSubmit={handleSubmitForm} className="space-y-4 pt-4">
              ...
              {/* Código de Barras del Producto (Visual en SVG, generado automáticamente, imprimible y escaneable) */}
              <BarcodeCard
                barcode={formData.barcode || formData.code}
                productName={formData.name || "Producto"}
                productPrice={formData.price}
                onChangeBarcode={(newBarcode) => setFormData({ ...formData, barcode: newBarcode, code: newBarcode })}
                onRegenerate={() => {
                  const newBarcode = generateProductBarcode();
                  setFormData({ ...formData, barcode: newBarcode, code: newBarcode });
                }}
              />
